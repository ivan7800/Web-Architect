...
const fail = (message) => { throw new Error(`[Production 4.1] ${message}`); };
...
expect(runtime.includes("const VERSION = '4.1.0'"), 'runtime Production Architect no está en 4.1.0');
expect(runtime.includes("const PREMIUM_VERSION = '4.1'"), 'falta versión Premium Output 4.1');
...
expect(runtime.includes('buildProductionOutput'), 'el gate no prepara la salida final antes de auditar');
...
expect(runtime.includes('premiumVersion: PREMIUM_VERSION'), 'API pública no expone Premium Output');
expect(runtime.includes('preview: buildProductionOutput'), 'API pública no expone preview auditable');
