...
## 3.1.0 — Mesa del Arquitecto

### Diseño
- Rediseño premium del estudio con concepto de mesa de dibujo técnico: doble retícula de plano (fina + maestra con crucetas), marcas de registro en el hero y capa ambiental `bg-aurora`.
- Tipografía en tres roles: display serif (títulos), sans (cuerpo) y mono técnica (eyebrows, badges, métricas, pasos). Sin webfonts: pilas de sistema, offline intacto.
- Eyebrows convertidos en rótulos de plano («Plano 01 / 05 · Brief»).

### Animaciones
- Orquestación de entrada del hero y la topbar con retardos escalonados.
- Contadores animados en las métricas del hero (1000 / 50 / 12).
- Anillo de puntuación cónico (`conic-gradient` + `@property`) sincronizado con la nota real del Quality Gate desde `app.js`.
- Transición de entrada entre pasos del asistente, escalonado de tarjetas del catálogo (`--i`), barrido de brillo en botones primarios, pulso vivo del indicador, shake en errores de validación y toast con muelle.
- Revelado por scroll con `IntersectionObserver` (progressive enhancement, activado solo si la API existe).
- Todo el movimiento se desactiva con `prefers-reduced-motion: reduce`.

### UX
- Raíl de pasos fijo (sticky) con estados hecho / actual / bloqueado más legibles y paso actual resaltado con el gradiente de marca.
- Botones «Volver» en los pasos 2–5 (`data-step-back`), enlazados a la navegación existente.
- Tarjeta seleccionada del catálogo marcada con «✓ elegido».
- Estados de foco y hover reforzados en inputs, chips y filas del editor de secciones.

### Correcciones
- E2E: el test de portada esperaba un heading `/crea modelos pro/i` inexistente; el nuevo H1 («Crea modelos pro, no plantillas.») lo hace verificable.
- El anillo del hero ya no es decorativo: refleja la puntuación calculada.


## 3.0.0 — Web Architect Studio

- Integración visual Universo 404 con 10 temas del estudio.
- Separación entre tema de aplicación y skin de salida.
- 12 arquitecturas HTML diferenciadas.
- Editor de secciones con orden, visibilidad, nombre, alta y eliminación.
- Guardado local e importación/exportación de proyectos v3.
- Exportación ZIP GitHub Pages sin dependencias de producción.
- Quality Gate calculado sobre HTML, contenido y contraste.
- Smoke test DOM para los nuevos flujos.
- README y metadatos actualizados.
